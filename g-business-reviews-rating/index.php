...
class google_business_reviews_rating
{
	...
	private function admin_current()
	{
		...
		return (isset($_GET['page']) && preg_match('/^google[\s_-]?business[\s_-]?reviews?[\s_-]?rating(?:[\s_-]?settings?)?$/i', $_GET['page']));
	}
	...
	public function admin_settings()
	{
		...
		$this->reviews_themes = array(
			...
			'columns two bubble' => __('Two Columns, Light Background, Bubble Outline', 'g-business-reviews-rating'),
			'columns two bubble fill' => __('Two Columns, Light Background, Bubble Filled', 'g-business-reviews-rating'),
			'columns two fonts' => __('Two Columns, Light Background with Fonts', 'g-business-reviews-rating'),
			'columns two fonts bubble' => __('Two Columns, Light Background with Fonts, Bubble Outline', 'g-business-reviews-rating'),
			'columns two fonts bubble fill' => __('Two Columns, Light Background with Fonts, Bubble Filled', 'g-business-reviews-rating'),
			'columns two dark' => __('Two Columns, Dark Background', 'g-business-reviews-rating'),
			'columns two dark bubble' => __('Two Columns, Dark Background, Bubble Outline', 'g-business-reviews-rating'),
			'columns two dark bubble fill' => __('Two Columns, Dark Background, Bubble Filled', 'g-business-reviews-rating'),
			'columns two dark fonts' => __('Two Columns, Dark Background with Fonts', 'g-business-reviews-rating'),
			'columns two dark fonts bubble' => __('Two Columns, Dark Background with Fonts, Bubble Outline', 'g-business-reviews-rating'),
			'columns two dark fonts bubble fill' => __('Two Columns, Dark Background with Fonts, Bubble Filled', 'g-business-reviews-rating'),
			'columns three' => __('Three Columns, Light Background', 'g-business-reviews-rating'),
			'columns three bubble' => __('Three Columns, Light Background, Bubble Outline', 'g-business-reviews-rating'),
			'columns three bubble fill' => __('Three Columns, Light Background, Bubble Filled', 'g-business-reviews-rating'),
			'columns three fonts' => __('Three Columns, Light Background with Fonts', 'g-business-reviews-rating'),
			'columns three fonts bubble' => __('Three Columns, Light Background with Fonts, Bubble Outline', 'g-business-reviews-rating'),
			'columns three fonts bubble fill' => __('Three Columns, Light Background with Fonts, Bubble Filled', 'g-business-reviews-rating'),
			'columns three dark' => __('Three Columns, Dark Background', 'g-business-reviews-rating'),
			'columns three dark bubble' => __('Three Columns, Dark Background, Bubble Outline', 'g-business-reviews-rating'),
			'columns three dark bubble fill' => __('Three Columns, Dark Background, Bubble Filled', 'g-business-reviews-rating'),
			'columns three dark fonts' => __('Three Columns, Dark Background with Fonts', 'g-business-reviews-rating'),
			'columns three dark fonts bubble' => __('Three Columns, Dark Background with Fonts, Bubble Outline', 'g-business-reviews-rating'),
			'columns three dark fonts bubble fill' => __('Three Columns, Dark Background with Fonts, Bubble Filled', 'g-business-reviews-rating'),
			'columns four' => __('Four Columns, Light Background', 'g-business-reviews-rating'),
			'columns four bubble' => __('Four Columns, Light Background, Bubble Outline', 'g-business-reviews-rating'),
			'columns four bubble fill' => __('Four Columns, Light Background, Bubble Filled', 'g-business-reviews-rating'),
			'columns four fonts' => __('Four Columns, Light Background with Fonts', 'g-business-reviews-rating'),
			'columns four fonts bubble' => __('Four Columns, Light Background with Fonts, Bubble Outline', 'g-business-reviews-rating'),
			'columns four fonts bubble fill' => __('Four Columns, Light Background with Fonts, Bubble Filled', 'g-business-reviews-rating'),
			'columns four dark' => __('Four Columns, Dark Background', 'g-business-reviews-rating'),
			'columns four dark bubble' => __('Four Columns, Dark Background, Bubble Outline', 'g-business-reviews-rating'),
			'columns four dark bubble fill' => __('Four Columns, Dark Background, Bubble Filled', 'g-business-reviews-rating'),
			'columns four dark fonts' => __('Four Columns, Dark Background with Fonts', 'g-business-reviews-rating'),
			'columns four dark fonts bubble' => __('Four Columns, Dark Background with Fonts, Bubble Outline', 'g-business-reviews-rating'),
			'columns four dark fonts bubble fill' => __('Four Columns, Dark Background with Fonts, Bubble Filled', 'g-business-reviews-rating')
		);
...
';
	...
	public function admin_ajax()
	{
		...
		$order = (isset($post['order']) && is_array($post['order'])) ? array_unique($post['order']) : array();
		$sort = (isset($post['sort']) && preg_match('/^reviews$/i', $section) && !preg_match('/^relevance(?:[_-]desc)?$/', $post['sort'])) ? preg_replace('/[^\w_-]/', '', strtolower($post['sort'])) : NULL;
		...
		$api_key = (isset($post['api_key']) && strlen($post['api_key']) >= 10 && strlen($post['api_key']) <= 255) ? $post['api_key'] : NULL;
		$place_id = (isset($post['place_id']) && strlen($post['place_id']) >= 10 && strlen($post['place_id']) <= 255) ? $post['place_id'] : NULL;
		$language = (isset($post['language']) && is_string($post['language']) && strlen($post['language']) >= 2) ? $post['language'] : NULL;
		$update = (isset($post['update']) && is_numeric($post['update'])) ? intval($post['update']) : NULL;
		...
		$import_type = (isset($post['import_type']) && preg_match('/^(?:original|translation)$/i', $post['import_type'])) ? $post['import_type'] : NULL;
	...
	public static function admin_add_plugin_meta($links, $file)
	{
		...
		$new_links = array(
			...
			'support' => '<a href="https://designextreme.com/wordpress/#google-business-reviews-rating" target="_blank" title="' . esc_attr__('Support', 'g-business-reviews-rating') . '">' . esc_html__('Support', 'g-business-reviews-rating') . '</a>'
		);
...
';
	...
	public function wp_display($atts = NULL, $content = NULL, $shortcode = NULL)
	{
		...
		$rel = (is_string($rel) && preg_match('/^\s*(?:author|bookmark|external|no(?:follow|referrer|opener))\s*$/i', $rel)) ? strtolower($rel) : ((preg_match('/^(?:f(?:alse)?|no?(?:ne)?|0|off|hide)$/i', $rel)) ? NULL : 'nofollow');
		...
		$stars = (is_null($stars) || is_bool($stars) && $stars || is_string($stars) && preg_match('/^(?:t(?:rue)?|y(?:es)?|1|on|show|svg|vector)$/i', $stars)) ? ((is_bool($summary) || is_array($summary) && (in_array('stars', $summary))) ? ((!array_key_exists('stars', $atts) && $color_scheme != NULL) ? 'css' : TRUE) : TRUE) : ((!array_key_exists('stars', $atts) && $color_scheme != NULL || is_string($stars) && preg_match('/(#(?:[0-9a-f]{2}){2,4}|#[0-9a-f]{3}|(?:rgba?|hsla?)\((?:\d+%?(?:deg|rad|grad|turn)?(?:,|\s)+){2,3}[\s\/]*[\d\.]+%?\))/i', $stars)) ? ((!array_key_exists('stars', $atts) && $color_scheme != NULL) ? 'css' : $stars) : ((is_string($stars) && preg_match('/^(?:html|css)$/i', $stars)) ? strtolower($stars) : FALSE));
		...
		$view = (is_numeric($view) && $view >= 1 && $view <= 50 && (is_numeric($limit) && $limit > 0 || !is_numeric($limit))) ? ((is_numeric($limit) && $limit > 0 && $view > $limit) ? intval($limit) : intval($view)) : NULL;
		...
		$bullet = (is_string($bullet) && mb_strlen($bullet) < 20) ? $bullet : NULL;
		...
		case 'google_reviews':
			...
			$review_item_inline = (is_string($review_item_order) && preg_match('/([\b\s_-]|^)inline([\b\s_-]|$)/i', $review_item_order));
			$review_item_text_first = (is_string($review_item_order) && preg_match('/([\b\s_-]|^)(?:review(?:[\b\s_-])?text|review|text)[\b\s_-]?(?:first|top|before|true|on|high|above|1)([\b\s_-]|$)/i', $review_item_order));
			$review_item_author_switch = (is_string($review_item_order) && preg_match('/([\b\s_-]|^)(?:author(?:[\b\s_-])?)[\b\s_-]?(?:last|bottom|after|low|below|switch|flip)([\b\s_-]|$)/i', $review_item_order));
			...
			$html = '<div '
			. (($id_name != NULL) ? 'id="' . esc_attr((($id_name != NULL) ? $id_name : 'google-business-reviews-rating' . (($this->instance_count > 1) ? '-' . $this->instance_count : ''))) . '" ' : '') 
...
';
				...
				{
					$author_name_capitalize = (is_string($name_format) && preg_match('/(?:^|\b)(?:capitali[sz]e|uc(?:first|words)|title(?:case))(?:\b|$)/i', $name_format));
					$author_name_lowercase = (!$author_name_capitalize && is_string($name_format) && preg_match('/(?:^|\b)lower(?:case)?(?:\b|$)/i', $name_format));
					$author_name_uppercase = (!$author_name_capitalize && !$author_name_lowercase && is_string($name_format) && preg_match('/(?:^|\b)upper(?:case)?(?:\b|$)/i', $name_format));
					...
					{
						$author_name_first = (isset($name_format_match[5]) && is_string($name_format_match[5]) && strtolower($name_format_match[5]) == 'first');
						$author_name_last = (!$author_name_first && isset($name_format_match[5]) && is_string($name_format_match[5]) && strtolower($name_format_match[5]) == 'last');
						$author_name_first_initials = (!$author_name_first && !$author_name_last && isset($name_format_match[1]) && is_string($name_format_match[1]) && strtolower($name_format_match[1]) == 'first');
						$author_name_last_initials = (!$author_name_first && !$author_name_last && !$author_name_first_initials && isset($name_format_match[1]) && is_string($name_format_match[1]) && strtolower($name_format_match[1]) == 'last');
...
';
					...
					foreach ($this->reviews_filtered as $i => $a)
					{
						...
						$html .= '		<' . $html_tags[4] . ' class="' . esc_attr('rating-' . $a['rating']) . ((is_numeric($view)) ? ' ' . (($index < $view) ? 'visible' : 'hidden') : '') . ((is_bool($avatar) && !$avatar) ? ' no-avatar' : '') . ((!is_bool($date) || is_bool($date) && !$date) && (!is_string($date) || is_string($date) && !is_numeric($a['time'])) ? ' no-date' : '') . (($review_item_text_first) ? ' text-first' : '') . (($review_item_inline) ? ' inline' : '') . (($review_item_author_switch) ? ' author-switch' : '') . '" data-index="' . esc_attr($index) . '">
';
						if (!$review_item_text_first)
						{
							if (!$review_item_author_switch)
							{
								if ((is_bool($avatar) && $avatar || is_string($avatar)) && (!is_string($theme) || is_string($theme) && !preg_match('/^badge(?:\s+.+)?$/', $theme)))
								{
									$html .= '			<span class="author-avatar">' . ((isset($a['author_url']) && $a['author_url'] != NULL && (is_bool($link_disable) && !$link_disable || is_array($link_disable) && !in_array('author', $link_disable))) ? '<a href="' . esc_attr($a['author_url']) . '" target="_blank"' . (($rel != NULL) ? ' rel="' . esc_attr($rel) . '"' : '') . '>' : '') . (($a['profile_photo_url'] != NULL) ? '<img src="' . esc_attr((is_string($avatar)) ? $avatar : $a['profile_photo_url']) . '" alt="Avatar">' : '—') . ((isset($a['author_url']) && $a['author_url'] != NULL && (is_bool($link_disable) && !$link_disable || is_array($link_disable) && !in_array('author', $link_disable))) ? '</a>' : '') . '</span>
';
								}
								
								if ($author_name != NULL)
								{
									$html .= '			<span class="author-name">' . ((isset($a['author_url']) && $a['author_url'] != NULL && (is_bool($link_disable) && !$link_disable || is_array($link_disable) && !in_array('author', $link_disable))) ? '<a href="' . esc_attr($a['author_url']) . '" target="_blank"' . (($rel != NULL) ? ' rel="' . esc_attr($rel) . '"' : '') . '>' : '') . esc_html($author_name) . ((isset($a['author_url']) && $a['author_url'] != NULL && (is_bool($link_disable) && !$link_disable || is_array($link_disable) && !in_array('author', $link_disable))) ? '</a>' : '') . '</span>
';
								}
							}
...
';
						...
						if ($review_text && !preg_match('/^\s*$/', $a['text']))
						{
							...
							if ($set_excerpt)
							{
								$html .= preg_replace('/(\r\n|\r|\n)+/', '<br>' . PHP_EOL . '				', preg_replace('/^(.{' . $review_text_excerpt_length . '}[^\s]{0,20})(.*)$/uis', '<span class="review-snippet">$1</span> <span class="review-more-placeholder">… ' . esc_html($more) . '</span><span class="review-full-text">$2</span>', esc_html(strip_tags($a['text']))));
							}
							else
							{
...
';
					...
					$html .= '	</' . $html_tags[3] . '>
';
					if (is_numeric($view) && $index > 0 && $view > 0 && $index >= $view)
					{
						$html .= '	<ul class="navigation">'; 
						
						for ($j = 0; $j < $index / $view; $j++)
						{
							$html .= '	  <li class="bullet' . (($j == 0) ? ' current' : '') . '"><a href="#' . esc_attr((($id_name != NULL) ? $id_name : 'google-business-reviews-rating' . (($this->instance_count > 1) ? '-' . $this->instance_count : ''))) . '" data-slide="' . esc_attr($j + 1) . '">' . (($bullet != NULL) ? $bullet : '●') . '</a></li>'; 
						}
...
';
			...
			if ((is_bool($attribution) && $attribution || is_string($attribution) && strlen($attribution) >= 1) && !preg_match('/\btiny\b/', $theme))
			{
				$html .= '	<' . $html_tags[6] . ' class="attribution"><span class="powered-by-google' . ((is_string($attribution)) ? ' ' . esc_attr($attribution) : '') . '" title="' . esc_attr__('Powered by Google') . '"></span></' . $html_tags[6] . '>
';
			}
