			...
			if (e != null) {
				...
				jQuery(e).each(function() {
					...
					reviews.push({
						...
						rating: (jQuery('.KdvmLc', this).length && jQuery('.KdvmLc:eq(0)', this).text().match(/^(\d+(?:\.\d+)?)\/(3|5|10|20|100)$/)) ? (Math.round(parseFloat(jQuery('.KdvmLc:eq(0)', this).text().replace(/^(\d+(?:\.\d+)?)\/(3|5|10|20|100)$/, '$1')) / parseFloat(jQuery('.KdvmLc:eq(0)', this).text().replace(/^(\d+(?:\.\d+)?)\/(3|5|10|20|100)$/, '$2'))*50)*0.1) : null,
				...
				if (reviews.length) {
					...
					jQuery('#html-import-input').after('<div id="html-import-output"><table id="reviews-import-table" class="wp-list-table widefat fixed striped reviews-table" data-order=""><thead></thead><tbody></tbody></table></div>');
					row = '<tr>\n'
...
function google_business_reviews_rating_preview(e) {
	...
	if (e != null && jQuery(e).attr('id').match(/^color[_-]?scheme[_-]?[a-z_-]+$/i) != null) {
		...
		if (typeof data.theme == 'string' && data.theme.length && typeof data.color_scheme == 'string' && data.color_scheme.length) {
			jQuery('#reviews-rating-preview').prop('class', data.theme + ' ' + data.color_scheme + ((jQuery('#reviews-rating-preview').hasClass('show')) ? ' show' : ''));
		}
		else if (typeof data.theme == 'string' && data.theme.length) {
			jQuery('#reviews-rating-preview').prop('class', data.theme + ((jQuery('#reviews-rating-preview').hasClass('show')) ? ' show' : ''));
		}
		else if (typeof data.color_scheme == 'string' && data.color_scheme.length) {
			jQuery('#reviews-rating-preview').prop('class', data.color_scheme + ((jQuery('#reviews-rating-preview').hasClass('show')) ? ' show' : ''));
		}
		else if (!jQuery('#reviews-rating-preview').hasClass('show')) {
			jQuery('#reviews-rating-preview').prop('class', false).removeAttr('class');
		}
...
function google_business_reviews_rating_widget(e, event) {
	if (typeof event == 'undefined' || event == null) {
		if (typeof jQuery(e).attr('id') != 'string' || jQuery(e).attr('id').match(/google[_-]?business[_-]?reviews[_-]?rating/i) == null || !jQuery('.google-business-reviews-rating', e).length) {
			return false
		}
	...
	var inputs = (jQuery(e).is(':input')) ? e : jQuery(':input', jQuery('.google-business-reviews-rating', e)),
		e = (jQuery(e).is(':input')) ? jQuery(e).closest('.google-business-reviews-rating') : jQuery('.google-business-reviews-rating', e);
	...
	jQuery(inputs).each(function() {
		...
		switch(jQuery(this).attr('name').replace(/^[^\[]+(?:\[\d+\])?\[([0-9a-z_-]+)\]$/i, '$1').replace(/-/g, '_')) {
		case 'theme':
			if (jQuery(this).val().match(/\bbadge|tiny\b/) == null) {
				break;
			}
			
			if (event != null && jQuery(':input:eq(0)', jQuery('.limit', e)).val().length && jQuery(':input:eq(0)', jQuery('.limit', e)).val().match(/^\d+$/) != null && parseInt(jQuery(':input:eq(0)', jQuery('.limit', e)).val()) >= 1) {
				jQuery(e).data('limit', jQuery(':input:eq(0)', jQuery('.limit', e)).val());
			}
			
			jQuery(':input:eq(0)', jQuery('.limit', e)).val(0);
		...
		case 'limit':
			if (jQuery(this).val().match(/^\d+$/) != null && parseInt(jQuery(this).val()) >= 1) {
				if (event != null  || (event == null && (jQuery(e).data('limit') == null || parseInt(jQuery(e).data('limit')) < 1))) {
					jQuery(e).data('limit', parseInt(jQuery(this).val()));
				}
		...
		case 'display_reviews':
			if (jQuery(this).is(':checked')) {
				...
				if (jQuery(':input:eq(0)', jQuery('.limit', e)).val().length && parseInt(jQuery(':input:eq(0)', jQuery('.limit', e)).val()) < 1) {
					if (parseInt(jQuery(e).data('limit')) >= 1) {
						jQuery(':input:eq(0)', jQuery('.limit', e)).val(jQuery(e).data('limit'));
					}
					else {
						jQuery(':input:eq(0)', jQuery('.limit', e)).val(1);
					}
				}
